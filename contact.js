document.addEventListener("DOMContentLoaded", () => {
  ...
  const initPreloader = () => {
    if (sessionStorage.getItem("preloaderShown") !== "true") {
      ...
      if (preloader) {
        ...
        document
          .querySelectorAll(".site-header, .hero-section, section, footer")
          .forEach((el) => {
  ...
  const initSmoothScroll = () => {
    const explorePackages = document.querySelector(
    ...
    if (explorePackages) {
      explorePackages.addEventListener("click", (e) => {
        ...
        const target = document.querySelector(
          explorePackages.getAttribute("href"),
        );
        if (target) {
          window.scrollTo({
  ...
  prevButton.addEventListener("click", () => {
    currentIndex =
      currentIndex === 0 ? testimonialCards.length - 1 : currentIndex - 1;
    showTestimonial(currentIndex);
  });

  nextButton.addEventListener("click", () => {
    currentIndex =
      currentIndex === testimonialCards.length - 1 ? 0 : currentIndex + 1;
    showTestimonial(currentIndex);
  });
...
// ✅ Collapsible Tabs (Getting Here section)
